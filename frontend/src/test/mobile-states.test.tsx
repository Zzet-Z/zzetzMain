import "@testing-library/jest-dom/vitest";
import { render, screen } from "@testing-library/react";
import { MemoryRouter, Route, Routes } from "react-router-dom";
import { expect, test } from "vitest";

import { SessionPage } from "../routes/session-page";

function renderSessionPage(initialState: {
  status: "queued" | "completed" | "expired" | "failed";
  queuePosition?: number;
}) {
  render(
    <MemoryRouter initialEntries={["/session/demo-token"]}>
      <Routes>
        <Route path="/session/:token" element={<SessionPage initialState={initialState} />} />
      </Routes>
    </MemoryRouter>,
  );
}

test("排队用户能看到等待文案", () => {
  renderSessionPage({ status: "queued", queuePosition: 1 });

  expect(screen.getByText(/当前正在为其他用户整理网站需求/)).toBeInTheDocument();
  expect(screen.getByText(/你前面还有 1 人/)).toBeInTheDocument();
});

test.each(["completed", "expired", "failed"] as const)(
  "%s 状态会禁用输入区",
  (status) => {
    renderSessionPage({ status });

    expect(screen.getByPlaceholderText("继续描述你的网站需求")).toBeDisabled();
    expect(screen.getByRole("button", { name: "发送" })).toBeDisabled();
  },
);

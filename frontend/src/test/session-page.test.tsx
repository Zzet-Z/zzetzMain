...
test("首次加载会话失败时展示后端错误提示而不是永久停留在加载态", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
  ...
  expect(await screen.findByText("读取会话失败")).toBeInTheDocument();
  expect(screen.queryByText("正在加载...")).not.toBeInTheDocument();
});

test("会优先展示后端返回的无效 token 中文提示", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes("/sessions/demo-token")) {
        return new Response(JSON.stringify({ message: "访问入口无效" }), { status: 404 });
      }
      return new Response(JSON.stringify({}), { status: 200 });
    }),
  );
  ...
  expect(await screen.findByText("访问入口无效")).toBeInTheDocument();
  expect(screen.queryByText("正在加载...")).not.toBeInTheDocument();
});

...
INVALID_PREVIOUS_DOCUMENT_MESSAGE = "上一版文档不存在，或还未生成完成。"
...
def create_token_session(
    ...
    *,
    ...
    now = now or utcnow()
    previous_document = validate_ready_document(
        db, previous_document_id=previous_document_id
    )
...
def validate_ready_document(db, *, previous_document_id: int | None) -> DocumentRecord | None:
    if previous_document_id is None:
        return None
    previous_document = db.get(DocumentRecord, previous_document_id)
    if previous_document is None or previous_document.status != "ready":
        raise ValueError(INVALID_PREVIOUS_DOCUMENT_MESSAGE)
